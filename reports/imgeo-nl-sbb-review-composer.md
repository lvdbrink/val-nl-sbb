...
## 1. Samenvatting
**Bron:** `tmp-imgeo.ttl` (lokaal Turtle-bestand)  
**Type invoer:** Turtle / TTL  
...
|---|---|---|---|---|
| W1 | 31 groepen dubbele `skos:prefLabel@nl` (o.a. `plus-type`×19, `niet-bgt`×18, `transitie`×13, `zand`×4, `bgt-type`×4) | Zelfde voorkeursterm voor verschillende begrippen in één kader | Dubbele voorkeurstermen bemoeilijken eenduidig gebruik en zoeken | Unieke, contextuele voorkeurstermen; context desnoods via `altLabel`/`scopeNote` of collectienaam |
...
| A3 | Schrijfwijze | Kapitalisatie van voorkeurstermen harmoniseren (ca. 55 Engels/CamelCase vs. overwegend Nederlands) | Consistentie in publicatie | Laag |
...
- **464** `skos:Concept`, **1** `skos:ConceptScheme`, **57** `skos:Collection`, **1043** RDF-subjecten, bestand ~547 KB
...
- **48** `isothes:broaderGeneric`-verwijzingen totaal (**39** naar niet-begrippen, **9** geldig); **9** `isothes:narrowerGeneric` (via `Registratiefgebied` en `Overigeconstructie`); **0** standaard `skos:broader`/`narrower`/`related`
...
- ConceptScheme heeft `rdfs:comment` met context over IMGeo/BGT
...
| SHACL-machinevalidatie tegen officieel NL-SBB-shapesbestand | Nee |
...
**Geanalyseerde bron:**
- **Type invoer:** Turtle (lokaal bestand)
- **Naam/bestandsnaam:** `tmp-imgeo.ttl`
...
*Deze skill is bedoeld voor beoordeling en advies. Er zijn geen wijzigingen doorgevoerd in het bronbestand.*
